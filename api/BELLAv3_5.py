...
def inputs(mensaje, server=None):
    if server is None: # Si no mandé nada por el server...
        return input(mensaje) # ...usa el teclado real.
    else:
        print(mensaje + str(server)) # ...si mandé algo, úsalo de una.
        return server
FILE_NAME = inputs("Para restaurar sesión (v3.4)(Server mode): ",server + ".xrp") # Cambiamos extensión a .xrp
...
def guardar_xor_pack_28():
    ...
    with open(filename_xrp, "wb") as f:
        for idx in activas:
            ...
            # El "Colchón": truncamos a 24 o rellenamos con ceros hasta 24
            texto_fijo = texto[:24].ljust(24, b'\x00')
            len_texto = len(texto)
            # Cambiamos H por I (presión) y mantenemos q (memoria signed)
            # Total: I(4) + B(1) + q(8) + I(4) = 17 bytes
            paquete = struct.pack(">IBqI", 
                idx, 
                0x9C, 
                int(cerebro_asm.mente[idx]), 
                int(exps[idx][2]) # Ahora aguanta hasta 4,294,967,295 de presión
            ) 
                                            
            f.write(paquete)
            f.write(texto_fijo) # El texto va justo después del header
    ...
    if os.path.exists(filename_xrp):
        ...
        with open(filename_xrp, "rb") as f:
            while True:
                ...
                header = f.read(17) # Ajustado al nuevo pack fijo
                if not header: break
                
                idx, sep, mem, pres= struct.unpack(">IBqI", header)
                
                # Leemos el colchón fijo de 24 bytes
                texto_raw = f.read(24)
                traductor[idx] = texto_raw.split(b'\x00')[0].decode('utf-8', errors ='ignore')
                
                # 3. Leemos el separador final (0xE8)
                f.read(1) # Saltamos el 0xE8
                
                # Reanimamos la neurona
                if idx < num_n:
                    cerebro_asm.mente[idx] = mem # Cambiamos otro vestigio de versiones pasadas
                    exps[idx] = [pres, pres, pres]

        fin_load = time.perf_counter()
        print(f"[xor pack 2.8]: Reanimada en {fin_load - inicio_load:.6f}s (Modo In-line)")        
    else:
        print("[xor pack]: Archivo no encontrado o vacío. Iniciando Tabula Rasa.")
...
num_n = 131072
cerebro_asm = XIP(buffer_size_mb=1)
...
if not cerebro_asm.cargar_mente(FILE_NAME):
...
def entrenar_con_voz(texto):
    for i in range(0, len(texto), 24):
        bloque = texto[i : i + 24]
        frags = [bloque[j : j + 24] for j in range(0, len(bloque), 8)]
        ...
        for objetivo_idx, fuerza, slot, contenido in pasos:
            ...
            if slot == 0: cerebro_asm.mente[objetivo_idx] += (fuerza << PosZ)
            elif slot == 1: cerebro_asm.mente[objetivo_idx] += (fuerza << PosY)
            else: cerebro_asm.mente[objetivo_idx] += (fuerza << PosX)
            ...
            n_gen, ex0, ex1, ex2 = Numeraso2_update(
                ...
                exps[objetivo_idx][1], 
                exps[objetivo_idx][2], 
                cerebro_asm.mente[objetivo_idx]
            )
